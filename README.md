...
동명대학교 게임 개발 동아리 연합이 여는 게임 개발 컨퍼런스 **CONTINUE?** 의 공식 홈페이지입니다.
...
| 구분 값 | 뜻 |
| --- | --- |
| `공통` | 전원이 함께 듣는 순서 (개회식, 키노트, 공통 세션, 네트워킹, 폐회) |
| `휴식` | 쉬는 시간·식사. 화면에서 흐리게 처리되고 상세 페이지를 만들지 않습니다 |
| 트랙 이름 | `트랙` 탭에 적은 이름 그대로 (기획 / 아트 / 프로그래밍) |

**시작 시각이 같은 줄은 화면에서 한 행에 나란히 놓입니다.** 오후 병렬 트랙이 이렇게 표현됩니다.
