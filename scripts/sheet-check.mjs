 ...
 */
import { TABS, tablesToContent, BREAK } from "./sheet-schema.mjs";
...
const sessions = content.schedule.filter((e) => e.kind !== BREAK);
const named = sessions.filter((e) => e.speaker !== "");

console.log("\n모든 탭이 정상입니다.\n");
console.log("  읽어 들인 내용");
for (const [label, n] of [
  ["트랙", content.tracks.length],
  ["일정 줄", content.schedule.length],
  ["세션", sessions.length],
  ["연사 확정", named.length],
  ["FAQ", content.faq.length],
]) {
  console.log(`    ${label.padEnd(10)} ${n}건`);
}

console.log(
  `\n  행사      ${content.site.name} · ${content.site.dateLabel} ${content.site.timeLabel}` +
    `\n  장소      ${content.site.venue}` +
    `\n  신청      ${content.site.applyUrl || "(주소 없음 — 버튼이 '준비 중'으로 보입니다)"}\n`,
);
